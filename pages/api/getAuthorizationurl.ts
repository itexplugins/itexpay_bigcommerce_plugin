...
export default async function getAuthorizationurl(req: NextApiRequest, res: NextApiResponse) {
    ...
    try {
        ...
        let options = {
            ...
            body: `{"amount": ${grand_total}, "currency": "NGN", "redirecturl": "http://localhost:3000/api/capture/${id}", "customer": {"email": "${email}", "first_name": "${first_name}", "last_name": "${last_name}", "phone_number": "${phone}" }, "reference": ${reference}}`,
